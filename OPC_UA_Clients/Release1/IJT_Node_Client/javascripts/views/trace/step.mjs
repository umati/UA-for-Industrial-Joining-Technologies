...
const PHYS_TORQUE = 2
const PHYS_ANGLE = 3
const PHYS_CURRENT = 11
const LIMIT_Y_OFFSET = 5
...
export default class Step {
  ...
  calculatePoints () {
    for (const value of this.values) {
      ...
      if ((this.xDimensionName === 'angle' && value.physicalQuantity === PHYS_TIME) ||
        (this.xDimensionName === 'time' && value.physicalQuantity === PHYS_ANGLE) ||
  ...
  interpretPoint (value) {
    ...
    let target
    if (xHigh !== null) {
      limits.push({ x: xHigh - this.displayOffset + xOffset, y: y + LIMIT_Y_OFFSET, name: '[limit]', type: 'limit' })
    }
    if (yHigh !== null) {
      limits.push({ x: x - this.displayOffset + xOffset, y: yHigh + LIMIT_Y_OFFSET, name: '[limit]', type: 'limit' })
    }
    const value2 = { x: x - this.displayOffset + xOffset, y, type: '' }

    if (xLow !== null) {
      limits.push({ x: xLow - this.displayOffset + xOffset, y: y + LIMIT_Y_OFFSET, name: '[limit]', type: 'limit' })
    }
    if (yLow !== null) {
      limits.push({ x: x - this.displayOffset + xOffset, y: yLow + LIMIT_Y_OFFSET, name: '[limit]', type: 'limit' })
    }
    if (xTarget !== null) {
      target = { x: xTarget - this.displayOffset + xOffset, y: y + LIMIT_Y_OFFSET, name: '[target]', type: 'target' }
    }
    if (yTarget !== null) {
      target = { x: x - this.displayOffset + xOffset, y: yTarget + LIMIT_Y_OFFSET, name: '[target]', type: 'target' }
    }
