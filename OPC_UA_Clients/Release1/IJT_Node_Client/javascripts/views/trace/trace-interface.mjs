export default class TraceInterface {
  ...
  updateTracesInGUI (allTraces) {
    ...
    for (const trace of allTraces) {
      ...
      tracebutton.classList.add('my-button')
  ...
  selectTrace (id) {
    for (const button of this.traceDiv.children) {
      if (button.resultId === id) {
        button.classList.add('my-button-selected')
      } else {
        button.classList.remove('my-button-selected')
      }
    }
  }
  ...
  addStepInGUI (step) {
    ...
    stepButton.classList.add('my-button')
  ...
  generateHTML (container) {
    ...
    function createTuple (name, right) {
      ...
      outer.classList.add('kv-pair')
      const title = document.createElement('div')
      title.classList.add('kv-key')
      title.innerText = name
      outer.appendChild(title)
      const content = document.createElement('div')
      content.classList.add('kv-value')
    ...
    function createSelector (container, name, values, standalone = false) {
      ...
      selector.classList.add('kv-select')
    ...
    backGround.classList.add('my-info-area')
    backGround.classList.add('trace-area')
    container.appendChild(backGround)

    const title = document.createElement('div')
    title.classList.add('my-header')
    title.innerText = 'Trace'
    backGround.appendChild(title)

    this.canvas = document.createElement('canvas')
    this.canvas.setAttribute('id', 'my-chart')
    backGround.appendChild(this.canvas)

    const interfaceArea = document.createElement('div')
    interfaceArea.classList.add('trace-button-area')
    container.appendChild(interfaceArea)

    const trace = document.createElement('div')
    trace.classList.add('my-info-area')
    ...
    this.zoomButton.classList.add('my-button')
    this.zoomButton.innerText = 'Zoom'
    trace.appendChild(this.zoomButton) */

    const view = document.createElement('div')
    view.classList.add('my-info-area')
    view.classList.add('thin-view')
