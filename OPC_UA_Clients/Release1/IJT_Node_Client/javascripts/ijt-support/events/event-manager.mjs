 ...
 *
 */
import { ijtLog } from '../ijt-support.mjs'
  ...
  constructor (connectionManager) {
    ...
    this.socketHandler.registerMandatory('subscribed event', (msg, context) => {
      if (msg?.result?.SourceName?.value) {
        ijtLog.info('Subscribed event triggered: ' + msg.result.SourceName.value)
      } else {
        ijtLog.info('Event lacking SourceName received')
      }
  ...
  receivedEvent (msg) {
    ...
    for (const item of this.callbacks) {
      if (item.filter && item.filter(payload)) {
        // ijtLog.info(`SUBSCRIPTION: ${item.subscriberDetails}`)
