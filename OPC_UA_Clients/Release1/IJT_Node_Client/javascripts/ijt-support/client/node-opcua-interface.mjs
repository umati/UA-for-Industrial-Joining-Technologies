...
async function readFile (filePath) {
  try {
    ...
    // ijtLog.info(data.toString())
    return data
  } catch (error) {
    ijtLog.error(`Got an error trying to read the file: ${error.message}`)
  }
}
...
async function writeFile (filePath, content) {
  try {
    ...
    ijtLog.error(`Got an error trying to write the file: ${error.message}`)
  }
}
...
export class NodeOPCUAInterface {
  constructor (io, attributeIds) {
    ijtLog.info('Establishing interface')
  ...
  setupSocketIO (OPCUAClient) {
    ...
    function displayFunction (msg) {
      // ijtLog.info(msg);
      ijtLog.info('status message: ' + msg)
      io.emit('status message', msg)
    }

    ijtLog.info('Establishing sockets')
    const io = this.io
    ...
    io.on('connection', (socket) => {
      ...
      socket.on('connect to', endpointUrl => {
        ...
        if (endpointUrl) {
          ...
          if (existingConnection && typeof existingConnection.isActive === 'function' && existingConnection.isActive()) {
            ijtLog.info('Skipping duplicate connect request for ' + endpointUrl)
            return
          }
      ...
      socket.on('subscribe event', (endpoint, msg, subscriberDetails) => {
        // ijtLog.info('Nodejs OPC UA client attempting to subscribe to ' + msg)
        const connectionObject = this.connectionList[endpoint]
        if (!connectionObject) {
          return
        }
...
class Connection {
  ...
  setupClient () {
    if (this.isActive()) {
      ijtLog.info('Ignoring duplicate setupClient call for ' + this.endpointUrl)
      return
    }
    const io = this.io
    ...
    client.on('backoff', (retryCount, delay) => {
      ijtLog.info(`Retry ${retryCount}: reconnecting to ${preferredEndpointUrl} in ${delay} ms`)
    })

    ;(async () => {
      try {
        ijtLog.info('Attempting to establish connection to ' + preferredEndpointUrl)
        await client.connect(preferredEndpointUrl)
        ijtLog.info('Connection established to endpoint ' + preferredEndpointUrl)
        io.emit('connection established', { endpointurl: endpointUrl })

        this.session = await client.createSession()
        ijtLog.info('*************** Session established. (' + preferredEndpointUrl + ')')
        ...
        ijtLog.info('Subscription established. (' + preferredEndpointUrl + ')')
        io.emit('subscription created', { endpointurl: endpointUrl })

        ijtLog.info('Handling Datatypes')
        ...
        ijtLog.info('Handling NameSpaces')
        io.emit('namespaces', { endpointurl: endpointUrl, namespaces })

        this.connectionState = 'connected'
        ijtLog.info('Connection and session established.')
      } catch (err) {
        this.connectionState = 'error'
        ijtLog.info('Failure during establishing connection to OPC UA server ', err)
    ...
    (async () => {
      try {
        ...
        if (bpr2.statusCode !== this.opcua.StatusCodes.Good) {
          ijtLog.info(`Cannot find the ${nodeId} object.  ${path} `)
          return
        }
  ...
  browse (callid, nodeId, details = false) {
    (async () => {
      try {
        ...
        await this.session.browse(nodeToBrowse,
          function (err, browseResult) {
            if (!err) {
              ...
              // ijtLog.info(reference.browseName);
              // });
              // ijtLog.info(nodeId)
              // ijtLog.info(browseResult)
        ...
        ijtLog.info('************  FAIL Browse call: ' + err.message + err)
    ...
    (async () => {
      ...
      try {
        ...
        theSession.call(methodToCall2, (err, results) => {
          if (err) {
            ijtLog.info('************  FAIL Method call (in callback): ' + err)
          } else {
            // ijtLog.info('Callresult: ' + results)
        ...
        ijtLog.info('************ FAIL method call: ' + err)
  ...
  async closeConnection () {
    ...
    this._closingPromise = (async () => {
      ...
      try {
        ijtLog.info('Terminate eventmonitoring')
        for (const monitor of this.eventMonitoringItems) {
          await settleCallback((done) => monitor.terminate(done))
          ijtLog.info('Eventmonitoring terminated ' + this.endpointUrl)
        }
      } catch (err) {
        ijtLog.info('************ FAIL to close down EventMonitor: ' + err)
      ...
      try {
        ijtLog.info('Unsubscribe')
        if (this.subscription) {
          await this.subscription.terminate()
          ijtLog.info('Unsubscribed')
        }
      } catch (err) {
        ijtLog.info('************ FAIL to unsubscribe: ' + err)
        this.io.emit('error message', { error: err, context: 'closedown', message: err.message })
      } finally {
      ...
      try {
        ijtLog.info('Closing session')
        if (this.session) {
          await this.session.close(true)
          ijtLog.info('Session closed')
        } else {
          ijtLog.info('Session already closed')
        }
      } catch (err) {
        ijtLog.info('************ FAIL to close session: ' + err)
        this.io.emit('error message', { error: err, context: 'closedown', message: err.message })
      } finally {
        this.session = null
      }
      ...
      try {
        ijtLog.info('Disconnect client. (' + this.endpointUrl + ')')
        if (this.client) {
          await this.client.disconnect()
        }
      } catch (err) {
        ijtLog.info('************ FAIL to disconnect client: ' + err)
        this.io.emit('error message', { error: err, context: 'closedown', message: err.message })
      } finally {
        this.client = null
      }
      ...
      ijtLog.info('Client disconnected.\n=========================================')
  ...
  async eventSubscription (fields, subscriberDetails) {
    ...
    ijtLog.info('eventsubscription reached ')

    eventMonitoringItem.on('initialized', () => {
      ijtLog.info('event_monitoringItem initialized (' + fields + ')')
    })
    ...
    eventMonitoringItem.on('changed', (events) => {
      (async () => {
        try {
          ...
          ijtLog.info('================= RECIEVED EVENT')
          for (let i = 0; i < events.length; i++) {
            // ijtLog.info('================= LOOPING OVER ARGUMENTS ' + i)
            // ijtLog.info('FIELD: ' + fields[i], '=', events[i].toString())
            // ijtLog.info(events[i].value.resultContent)

            if (fields[i] === 'Result') {
              // ijtLog.info('=================================================')
              // ijtLog.info('======EVENT RESULT VALUE' + events[i].value)
              // ijtLog.info('=================================================')
              if (events[i] && events[i].value) {
                await this.opcua.promoteOpaqueStructure(this.session, [{ value: events[i].value }])
              }
              // ijtLog.info('------------ Opaque structure promoted')
              // ijtLog.info(events[i].value)
            }
          ...
          // ijtLog.info('eventsubscription triggered  (' + this.endpointUrl + ')')
