...
def _get_instructions(keyword: str) -> list[str]:
    ...
    return [
        ...
        if re.match(rf"^\s*{keyword}\b", line, re.IGNORECASE)
    ]
...
class TestDockerfileInstructions:
    ...
    def test_workdir_is_app(self):
        ...
        assert any("/app" in wd for wd in workdirs), (
            f"Expected WORKDIR /app, found: {workdirs}"
        )
    ...
    def test_expose_includes_port_3000(self):
        ...
        assert "3000" in all_ports, (
            f"Port 3000 not found in EXPOSE instructions: {exposes}"
        )
    ...
    def test_expose_includes_port_8001(self):
        ...
        exposes = _get_instructions("EXPOSE")
        all_ports = " ".join(exposes)
        assert "8001" in all_ports, (
            f"Port 8001 not found in EXPOSE instructions: {exposes}"
        )
    ...
    def test_user_instruction_present_for_non_root(self):
        ...
        assert len(users) >= 1, (
            "No USER instruction found in Dockerfile — container would run as root"
        )
    ...
    def test_user_is_not_root(self):
        ...
        for user_line in users:
            assert "root" not in user_line.lower(), (
                f"USER instruction must not use root: {user_line}"
            )
            assert "uid=0" not in user_line.lower(), (
                f"USER instruction must not use UID 0: {user_line}"
            )
    ...
    def test_production_stage_uses_setup_project(self):
        ...
        assert has_setup, (
            f"No CMD with setup_project.py found. CMDs: {cmds}"
        )
    ...
    def test_healthcheck_present(self):
        ...
        assert len(healthchecks) >= 1, (
            "No HEALTHCHECK instruction found — Docker cannot detect container readiness"
        )
    ...
    def test_multi_stage_build_has_test_and_production_stages(self):
        ...
        assert "test" in stages_text, (
            "No 'test' stage found in multi-stage build"
        )
        assert "production" in stages_text, (
            "No 'production' stage found in multi-stage build"
        )
...
class TestDockerCompose:
    ...
    def test_compose_has_ijt_web_client_service(self):
        ...
        assert "ijt_web_client" in services, (
            f"ijt_web_client service not found. Services: {list(services.keys())}"
        )
    ...
    def test_compose_ijt_web_client_exposes_port_3000(self):
        ...
        assert "3000" in ports_str, (
            f"Port 3000 not in ijt_web_client service ports: {ports}"
        )
    ...
    def test_compose_ijt_web_client_exposes_port_8001(self):
        ...
        ports = content["services"]["ijt_web_client"].get("ports", [])
        ports_str = " ".join(str(p) for p in ports)
        assert "8001" in ports_str, (
            f"Port 8001 not in ijt_web_client service ports: {ports}"
        )
