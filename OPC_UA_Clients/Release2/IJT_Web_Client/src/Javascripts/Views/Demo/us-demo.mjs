import { TraceInterface } from '../trace/trace-interface.mjs' // Buttons and other standard graphical elements
import ZoomHandler from '../trace/zoom-handler.mjs' // Handling different types of zoom
import TraceDisplay from '../trace/trace-display.mjs' // The trace area
...
import IJTPropertyView from './ijt-property-view.mjs' // The machine properties view
  ...
  */
  ...
  selectJoiningProcess (process) {
    ...
    this.methodManager.call(selectJoiningProcessMethod, values).then(
      ...
      (fail) => {
        ijtLog.error(JSON.stringify(fail))
      }
    )
  }
