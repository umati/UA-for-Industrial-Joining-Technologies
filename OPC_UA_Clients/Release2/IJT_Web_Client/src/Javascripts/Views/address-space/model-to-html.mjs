...
export default class ModelToHTML {
  ...
  toHTML (model, brief = true, parentName) {
    const toggler = (shortVersion, longVersionOuter) => {
    ...
    if (model.constructor.name === 'DefaultNode' && model.value && model.value.value !== null && model.value.value !== undefined) {
      let result = ''
    ...
    if (parentName) {
      ...
      ownerName.textContent = `${parentName}${abbreviation} \u25B7`
    ...
    combined.expandLong = () => { // This is used to get the initial display expanded
      if (longVersionOuter && shortVersion) {
        longVersionOuter.style.display = 'block'
        shortVersion.style.display = 'none'
      }
    }
  ...
  /**
 * this function takes a received object from the webclient and tries to create resonable HTML.
 ...
 * @param {} receivedObject a JSON representation of what was received
 ...
 */
  display (receivedObject, startText = 'Response') {
    ...
    if (typeof receivedObject === 'object') { // Handle a model
      ...
      onScreen.textContent = `${startText} ${this.prettyPrint(JSON.parse(receivedObject))}`
