...
export class WebSocketManager {
  ...
  establishWebSocket (establishedCallback, websocketUrl) {
    ...
    this.websocket.onopen = () => {
      ...
      while (this._sendQueue.length > 0) {
        ...
        try { this.websocket.send(queued) } catch (e) { console.error('Flush send failed:', e) }
      }
    }
  ...
  _scheduleReconnect () {
    ...
    console.log(`WebSocket reconnect in ${Math.round(delay + jitter)} ms (attempt ${this._reconnectAttempt})`)
    setTimeout(() => {
      this._reconnecting = false
      this.establishWebSocket(this.establishedCallback, this.websocketUrl)
    }, delay + jitter)
  ...
  send (command, endpoint, uniqueId, event) {
    ...
    if (!this.websocket || this.websocket.readyState !== WebSocket.OPEN) {
      console.log('WebSocket not ready — queuing message and triggering reconnect')
