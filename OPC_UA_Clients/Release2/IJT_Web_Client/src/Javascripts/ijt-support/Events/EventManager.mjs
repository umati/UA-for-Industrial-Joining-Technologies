...
export class EventManager {
  ...
  receivedEvent (message) {
    if (this.queue) {
      ...
      try {
        ...
        console.error('EventManager.receivedEvent error:', err)
      }
    }
  }
  ...
  makeCalls (message) {
    ...
    try {
      ...
      console.error('EventManager: createModelFromEvent failed:', err)
      return null
    }
    for (const item of this.callbacks) {
      try {
        ...
        console.error(`EventManager callback "${item.subscriberDetails ?? '?'}" threw:`, err)
      }
    }
    return eventModel
  }
