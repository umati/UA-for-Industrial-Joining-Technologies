...
export default class ZoomHandler {
  ...
  onclick = (evt, coord, resultId, stepId) => {
    if (resultId) {
      ...
      if (amountOfXChange > ZOOM_THRESHOLD && amountOfYChange > ZOOM_THRESHOLD) { // prevent single click zoom
  ...
  touchmove (evt, coord, touchOffset) {
    const modifyWithOffset = (pointList) => {
      const returnList = []
    ...
    const areaRatios = (zoomArea, screen) => {
      return {
        x: {
