...
/// <summary>
/// Security-flow matrix for the C# client against the IJT Server Simulator.
/// The runner selects the SUT with IJT_SUT=windows|linux and assigns the cell port.
/// </summary>
[Collection("LiveServer")]
[Trait("Category", "Live")]
[Trait("Category", "SecurityMatrix")]
public sealed class SecurityMatrixTests(OpcUaServerFixture fixture)
{
        ...
        string caseName)
    {
        ...
        using var scope = SecurityMatrixTempScope.Create(caseName);
        ...
        string caseName)
    {
        ...
        Skip.If(
            ...
            "(asyncua) console matrix (B1) end-to-end on the same server, confirming the " +
            "server allows the read and the credentials are correct. Tracked in " +
            "session plan.md as 'A1/A2 NamespaceArray' (Bucket A).");

        var users = LoadUsers();
        using var scope = SecurityMatrixTempScope.Create($"UserName_HappyPath_{caseName}");
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(30));
        await using var session = await ConnectWithServerAutoTrustRetryAsync(
            BuildConfig(
                scope,
        ...
        string caseName)
    {
        ...
        var users = LoadUsers();
        using var scope = SecurityMatrixTempScope.Create($"UserName_WrongPassword_{caseName}");
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(30));
        ...
        string caseName)
    {
        ...
        var users = LoadUsers();
        using var scope = SecurityMatrixTempScope.Create($"UserName_UnknownUser_{caseName}");
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(30));
        await AssertConnectFailsWithAnyStatusAsync(
            BuildConfig(
                scope,
        ...
        string caseName)
    {
        ...
        using var scope = SecurityMatrixTempScope.Create($"X509IdentityToken_CertificateUserTokenPolicy_{caseName}");
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(30));
        await using var session = await ConnectWithServerAutoTrustRetryAsync(
            BuildConfig(scope, securityPolicyUri, securityMode, UserIdentityKind.Anonymous),
            cts.Token).ConfigureAwait(false);
        ...
        string caseName)
    {
        ...
        using var scope = SecurityMatrixTempScope.Create($"X509IdentityToken_KnownThumbprint_user1_{caseName}");
        Skip.If(
        ...
        Skip.If(
            ...
            "session activation succeeds. Tracked in session plan.md as 'A1/A2 NamespaceArray' " +
            "(Bucket A).");
    ...
    public async Task X509IdentityToken_UnknownUser_IsRejected()
    {
        ...
        using var scope = SecurityMatrixTempScope.Create("X509IdentityToken");
    ...
    private ClientConfig BuildConfig(
        ...
        => new()
        {
            ...
            ApplicationName = $"IJT CSharp Security Matrix {MatrixCellName()}",
    ...
    private List<EndpointDescription> DiscoverEndpoints()
    {
        ...
        {
            ApplicationName = $"IJT CSharp Security Matrix Discovery {MatrixCellName()}",
        ...
        CancellationToken ct)
    {
        ...
        Assert.Contains(serviceResult!.StatusCode, expectedStatusCodes);
    }
    ...
    private static string MatrixCellName()
        => Environment.GetEnvironmentVariable("IJT_SECURITY_MATRIX_CELL") ?? "local";

    private static SecurityMatrixUsers LoadUsers()
    {
        ...
        if (string.IsNullOrWhiteSpace(path) || !File.Exists(path))
            throw new FileNotFoundException("Security matrix users file was not found.", path);
        ...
        return new SecurityMatrixUsers(
        ...
        string section)
    {
        if (!sections.TryGetValue(section, out var values)
            ...
            throw new InvalidOperationException($"Security matrix users file is missing {section}.username/password.");
    ...
    private sealed record SecurityMatrixUsers(
        Credentials Positive,
        Credentials WrongPassword,
        Credentials UnknownUser);

    private sealed class SecurityMatrixTempScope : IDisposable
    {
        private SecurityMatrixTempScope(string rootPath)
        {
            RootPath = rootPath;
        ...
        public static SecurityMatrixTempScope Create(string caseName)
        {
            ...
            return new SecurityMatrixTempScope(root);
        }

        public string WriteUserCertificatePfx()
            => WriteUserCertificatePfx("IJT Security Matrix X509 User");
        ...
        private static string ResolveProjectTempRoot()
        {
            ...
            while (dir is not null)
            {
                if (File.Exists(Path.Combine(dir.FullName, "IJT_CSharp_Client.csproj")))
                    return Path.Combine(dir.FullName, "tmp", "security-matrix");
                ...
                if (File.Exists(Path.Combine(projectRoot, "IJT_CSharp_Client.csproj")))
                    return Path.Combine(projectRoot, "tmp", "security-matrix");

                dir = dir.Parent;
            }

            return Path.Combine(AppContext.BaseDirectory, "tmp", "security-matrix");
        }
