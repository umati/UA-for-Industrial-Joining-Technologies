...
async def test_joining_system_event_type_exists(session_client, ns_indices):
    ...
    try:
        ...
        pytest.fail(
            f"JoiningSystemEventType (ns={ns_ijt}; i={IJTTypes.JOINING_SYSTEM_EVENT_TYPE}) is not readable: {exc}"
        )
...
# ---------------------------------------------------------------------------
async def test_result_ready_inherits_from_joining_system_event(session_client, ns_indices):
    ...
    result_ready_node = session_client.get_node(ua.NodeId(IJTTypes.JOINING_SYSTEM_RESULT_READY_EVENT_TYPE, ns_ijt))
    refs = await result_ready_node.get_references(
        refs=ua.NodeId(RefTypes.HAS_SUBTYPE, ns_indices[NS_OPC_UA]),
        direction=ua.BrowseDirection.Inverse,
    )
    if not refs:
        pytest.skip("No inverse HasSubtype references found on ResultReadyEventType — cannot verify inheritance")
