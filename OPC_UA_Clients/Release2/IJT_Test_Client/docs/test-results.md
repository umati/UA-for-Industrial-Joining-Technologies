...
| `test-results/cu-coverage-report.json` | JSON | Machine-readable CU compliance report by test, conformance unit, raw support outcome, conservative compliance status, and workbook row traceability |
| `test-results/cu-coverage-report-unit.json` | JSON | Unit-stage plugin output; kept separate so the main CU report name is reserved for live compliance/conformance runs |
