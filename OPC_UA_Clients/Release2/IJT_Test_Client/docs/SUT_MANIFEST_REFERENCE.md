...
| `workflows.max_start_invocations_by_result_classification` | mapping | No | `{single: 1, batch: 3, sync: 3, job: 6}` | - | Maximum accepted StartSelectedJoining calls while waiting for each result classification. Supported keys are single, batch, sync, and job. Every value is also capped by max_start_invocations. |
...
| `workflows.process_selector.policy` | str | No | `first_compatible` | `all_compatible`, `exact_match`, `first_available`, `first_compatible`, `first_ready` | Joining process selection policy. Non-exact policies order direct standard-classification matches first and may then try a Job process as bounded evidence for Batch or Sync results; only a completed requested-classification result proves support. all_compatible exercises every ordered candidate, while the other non-exact policies stop after the first proof. exact_match pins an advertised identifier; a result-specific exact selector may intentionally cross process-classification metadata, but still requires result-stream proof. Names and vendor fields never prove capability. |
...
| `workflows.expected_results.referenced_child_completion_policy` | str | No | `terminal_required` | `partial_allowed`, `terminal_required` | How captured results resolve a referenced child ResultId. terminal_required waits for IsPartial=false; partial_allowed accepts the latest partial update for open or effectively infinite batches. |
...
| `workflows.evidence_reuse.enabled` | bool | No | `false` | - | Reuse one provenance-keyed capture across compatible CU assertions. |
| `workflows.evidence_reuse.scope` | str | No | `current_run` | `current_run` | Evidence lifetime. current_run never persists evidence across runner invocations. |
...
| `triggers.event.mode` | str | No | `observe_only` | `manual_trigger`, `none`, `observe_only`, `simulate_methods`, `workflow_actions` | Event trigger mode. |
| `triggers.event.actions` | mapping | No | `{}` | - | Event CU key to approved workflow name. Used only by workflow_actions mode. |
...
| `execution_policy.counter_effects` | list of mappings | No | `[]` | - | Explicit counter-effect scenarios. Every entry has these fields: method is the OPC UA BrowseName; count is a positive bounded change; process_policy must be exact_match because physical counter mutations may affect only one explicitly reviewed process; joining_process_id pins that current discovered process; expected_result_classification optionally requires a partial or final affected-result update that explicitly references the completed InterventionResult; accepted values are single, batch, sync, job, stitching, intervention, or text. |
...
| `execution_policy.identifier_workflows.enabled` | bool | No | `false` | - | Allow approved identifier workflows to mutate test-owned identifiers. |
| `execution_policy.identifier_workflows.value_policy` | str | No | `run_unique` | `run_unique` | How identifier workflows create collision-safe values. |
| `execution_policy.identifier_workflows.cleanup_policy` | str | No | `selective_test_owned_only` | `selective_test_owned_only` | Cleanup may reset only identifiers created by the current test workflow. |
| `execution_policy.identifier_workflows.allow_reset_all` | bool | No | `false` | - | Allow broad ResetAll only through the separately approved reset_all_identifiers workflow; never use it as cleanup. |
| `execution_policy.request_results.filter_strategy` | str | No | `sequence_number` | `both`, `sequence_number`, `timestamp` | RequestResults filtering mode: sequence_number (default and preferred), timestamp, or both. Timestamp mode transmits both sequence arguments as zero. Both mode populates all five arguments, but conforming servers use timestamps only when both sequence arguments are zero (OPC 40450-1). |
| `execution_policy.request_results.from_sequence_number` | int | No | `1` | - | Configured inclusive lower sequence bound for sequence_number or both mode (1..UInt64 max). Active result tests adjust the transmitted window when needed to include the result they just triggered. Timestamp mode ignores this setting and transmits 0. |
| `execution_policy.request_results.to_sequence_number` | int | No | `50` | - | Configured inclusive upper sequence bound for sequence_number or both mode (at least from_sequence_number, up to UInt64 max; default: 50, providing a safe 50-result compliance window). Active result tests dynamically shift this window to cover recent history ending at the triggered sequence number. Timestamp mode ignores this setting and transmits 0. |
| `execution_policy.request_results.from_time` | str | No | `'2000-01-01T00:00:00Z'` | - | Inclusive lower timestamp for timestamp or both mode, as ISO 8601. Use Z or an explicit UTC offset; a timestamp without an offset is interpreted as UTC. When both timestamps are valid, from_time must not be later than to_time. Invalid text uses the safe runtime lower default. |
| `execution_policy.request_results.to_time` | str | No | `'9999-01-01T00:00:00Z'` | - | Inclusive upper timestamp for timestamp or both mode, as ISO 8601. Use Z or an explicit UTC offset; a timestamp without an offset is interpreted as UTC. Invalid text uses the safe runtime upper default. Conforming servers ignore both timestamps when sequence bounds are non-zero. |
| `execution_policy.request_results.min_duration_ms` | number | No | `100.0` | - | RequestedMinimumDurationBetweenResults as a finite, non-negative number of milliseconds. It is sent as the OPC UA Duration/Double argument; 100.0 ms is the recommended default interval. |
