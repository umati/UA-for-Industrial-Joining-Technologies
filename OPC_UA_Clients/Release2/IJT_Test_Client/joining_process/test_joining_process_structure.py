...
async def test_joining_process_management_exists(joining_process_management):
    ...
    assert joining_process_management is not None, "JoiningProcessManagement node must not be None"


async def test_joining_process_management_type_correct(joining_process_management, ns_indices):
    ...
    assert type_def is not None, "JoiningProcessManagement node has no HasTypeDefinition reference"
...
async def test_select_joining_process_method_exists(joining_process_management, ns_indices):
    # §11.1 SelectJoiningProcess method must be present (IJT Base ns)
    ns_ijt = ns_indices.get(NS_IJT_BASE)
    if ns_ijt is None:
        pytest.skip("IJT Base namespace not registered on server")
    node = await find_child_by_browse_name(joining_process_management, BN.SELECT_JOINING_PROCESS, ns_ijt)
    assert node is not None, f"Method '{BN.SELECT_JOINING_PROCESS}' not found in JoiningProcessManagement (ns={ns_ijt})"


async def test_start_selected_joining_method_exists(joining_process_management, ns_indices):
    # §11.1 StartSelectedJoining method must be present (IJT Base ns)
    ns_ijt = ns_indices.get(NS_IJT_BASE)
    if ns_ijt is None:
        pytest.skip("IJT Base namespace not registered on server")
    node = await find_child_by_browse_name(joining_process_management, BN.START_SELECTED_JOINING, ns_ijt)
    assert node is not None, f"Method '{BN.START_SELECTED_JOINING}' not found in JoiningProcessManagement (ns={ns_ijt})"
...
async def test_enable_asset_method_exists(asset_management, ns_indices):
    ...
    assert node is not None, f"Method '{BN.ENABLE_ASSET}' not found in AssetManagement MethodSet (ns={ns_ijt})"
...
async def test_abort_joining_process_method_exists(joining_process_management, ns_indices):
    # §11.1 AbortJoiningProcess method must be present (IJT Base ns)
    ns_ijt = ns_indices.get(NS_IJT_BASE)
    if ns_ijt is None:
        pytest.skip("IJT Base namespace not registered on server")
    node = await find_child_by_browse_name(joining_process_management, BN.ABORT_JOINING_PROCESS, ns_ijt)
    assert node is not None, f"Method '{BN.ABORT_JOINING_PROCESS}' not found in JoiningProcessManagement (ns={ns_ijt})"
