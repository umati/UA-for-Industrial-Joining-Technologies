...
async def test_results_folder_exists(result_management, ns_indices):
    ...
    assert node is not None, f"Expected child '{BN.RESULTS}' (ns={ns_mr}) under ResultManagement"
...
async def test_get_latest_result_method_exists(result_management, ns_indices):
    ...
    assert node is not None, f"Method '{BN.GET_LATEST_RESULT}' must exist in ResultManagement (ns_mr={ns_mr})"
...
async def test_request_results_method_exists(result_management, ns_indices):
    ...
    assert node is not None, f"Method '{BN.REQUEST_RESULTS}' must exist in ResultManagement (ns_ijt_base={ns_ijt_base})"
...
async def test_simulate_job_result_method_exists(simulate_results_folder, ns_indices):
    ...
    assert node is not None, f"Method '{BN.SIMULATE_JOB_RESULT}' must exist in SimulateResults folder (ns_app={ns_app})"
