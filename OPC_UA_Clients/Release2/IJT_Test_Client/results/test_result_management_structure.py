...
async def test_result_management_exists(result_management):
    assert result_management is not None, "ResultManagement node must not be None"
...
async def test_get_latest_result_method_exists(result_management, ns_indices):
    ...
    node = await find_child_by_browse_name(
        result_management, BN.GET_LATEST_RESULT, ns_mr
    )
    assert node is not None, (
...
async def test_get_result_by_id_method_exists(result_management, ns_indices):
    ns_mr = ns_indices[NS_MACH_RESULT]
    node = await find_child_by_browse_name(
        result_management, BN.GET_RESULT_BY_ID, ns_mr
    )
    assert node is not None, (
...
async def test_get_result_id_list_filtered_method_exists(result_management, ns_indices):
    ns_mr = ns_indices[NS_MACH_RESULT]
    node = await find_child_by_browse_name(
        result_management, BN.GET_RESULT_ID_LIST_FILTERED, ns_mr
    )
    assert node is not None, (
...
async def test_release_result_handle_method_exists(result_management, ns_indices):
    ns_mr = ns_indices[NS_MACH_RESULT]
    node = await find_child_by_browse_name(
        result_management, BN.RELEASE_RESULT_HANDLE, ns_mr
    )
    assert node is not None, (
...
async def test_request_results_method_exists(result_management, ns_indices):
    ...
    node = await find_child_by_browse_name(
        result_management, BN.REQUEST_RESULTS, ns_ijt_base
    )
    assert node is not None, (
...
# ---------------------------------------------------------------------------
async def test_simulate_single_result_method_exists(
    simulate_results_folder, ns_indices
):
    ns_app = ns_indices[NS_APP]
    node = await find_child_by_browse_name(
        simulate_results_folder, BN.SIMULATE_SINGLE_RESULT, ns_app
    )
    assert node is not None, (
...
async def test_simulate_batch_or_sync_method_exists(
    simulate_results_folder, ns_indices
):
    ns_app = ns_indices[NS_APP]
    node = await find_child_by_browse_name(
        simulate_results_folder, BN.SIMULATE_BATCH_OR_SYNC_RESULT, ns_app
    )
    assert node is not None, (
...
async def test_simulate_job_result_method_exists(simulate_results_folder, ns_indices):
    ns_app = ns_indices[NS_APP]
    node = await find_child_by_browse_name(
        simulate_results_folder, BN.SIMULATE_JOB_RESULT, ns_app
    )
    assert node is not None, (
