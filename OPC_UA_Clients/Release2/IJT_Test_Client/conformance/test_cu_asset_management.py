...
"""
import pytest
from helpers.namespaces import NS_IJT_BASE, NS_DI, NS_MACHINERY, BN, IJTTypes
...
async def test_cu_asset_management_asset_management_addin(asset_management):
    ...
    assert asset_management is not None, (
        "AssetManagement AddIn node must not be None"
    )
...
async def test_cu_asset_management_tool_has_required_interfaces(tools_instances, ns_indices):
    ...
    has_tool_iface = await has_interface(
        tool_node, ns_ijt, IJTTypes.ITOOL_TYPE
    )
