...
async def _instances_from(folder, label: str):
    ...
    assert instances, f"{label} folder has no instances — required by current server configuration"
    return instances
...
async def test_controller_interface_node_ids_are_in_ijt_base_namespace(controllers_instances, ns_indices):
    ...
    assert iface_ids, f"No HasInterface references found on '{name}' — server must emit HasInterface per IJT spec"
...
async def test_tool_parameters_implement_i_tightening_tool_parameters_type(tools_instances, ns_indices):
    ...
    for name, node in tools_instances:
        ...
        if params is None:
            pytest.fail(f"Tool '{name}' has no Parameters folder — Parameters is required for Tool instances")
...
async def test_all_asset_identification_implements_i_joining_additional_information_type(assets_folder, ns_indices):
    ...
    for folder in all_folders:
        ...
        for name, node in instances:
            ...
            checked += 1
    assert checked > 0, (
        "No asset instances with Identification nodes found — Identification is mandatory for all asset instances"
    )
