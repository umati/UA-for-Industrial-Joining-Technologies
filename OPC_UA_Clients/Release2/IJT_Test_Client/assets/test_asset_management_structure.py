...
async def test_joining_system_browse_name_is_string(joining_system):
    ...
    assert bn.Name and len(bn.Name) > 0, "JoiningSystem browse name must not be empty"
...
async def test_all_13_asset_folders_exist(folder_name, assets_folder, ns_indices):
    ...
    assert folder_node is not None, f"Required asset folder '{folder_name}' not found under Assets"
...
async def test_controllers_folder_not_empty(controllers_instances):
    ...
    assert len(controllers_instances) > 0, "No controller instances found in Controllers folder"


async def test_tools_folder_not_empty(tools_instances):
    """Tools folder must contain at least one tool instance."""
    assert len(tools_instances) > 0, "No tool instances found in Tools folder"
...
async def test_asset_management_has_method_set(asset_management, ns_indices):
    ...
    assert method_set is not None, "MethodSet node not found under AssetManagement (IJT Base namespace)"
...
async def test_asset_instances_have_unique_browse_names(controllers_folder):
    ...
    assert len(names) == len(set(names)), f"Duplicate browse names detected in Controllers folder: {names}"
