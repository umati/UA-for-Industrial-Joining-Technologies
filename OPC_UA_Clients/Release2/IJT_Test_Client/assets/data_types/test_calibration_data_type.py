...
async def test_calibration_has_last_calibration(tools_instances, ns_indices):
    ...
    last_cal_node = await find_child_by_browse_name(
        calibration_node, "LastCalibration", ns_ijt
    )
    assert last_cal_node is not None, (
        "LastCalibration node not found under Calibration"
    )
...
async def test_calibration_has_calibration_place(tools_instances, ns_indices):
    ...
    if calibration_node is None:
        pytest.skip("Calibration node not found on first tool")
    place_node = await find_child_by_browse_name(
        calibration_node, "CalibrationPlace", ns_ijt
    )
    assert place_node is not None, (
        "CalibrationPlace node not found under Calibration"
    )
    value = await place_node.read_value()
    assert isinstance(value, str), (
        f"CalibrationPlace value expected str, got {type(value).__name__}: {value!r}"
    )
async def test_calibration_has_next_calibration(tools_instances, ns_indices):
    ...
    if calibration_node is None:
        pytest.skip("Calibration node not found on first tool")
    next_cal_node = await find_child_by_browse_name(
        calibration_node, "NextCalibration", ns_ijt
    )
    assert next_cal_node is not None, (
        "NextCalibration node not found under Calibration"
    )
...
async def test_calibration_has_calibration_value(tools_instances, ns_indices):
    ...
    if calibration_node is None:
        pytest.skip("Calibration node not found on first tool")
    cal_value_node = await find_child_by_browse_name(
        calibration_node, BN.CALIBRATION_VALUE, ns_ijt
    )
    assert cal_value_node is not None, (
        "CalibrationValue node not found under Calibration"
    )
...
async def test_calibration_has_engineering_units(tools_instances, ns_indices):
    ...
    assert eu_node is not None, (
        "EngineeringUnits node not found under CalibrationValue "
        "(tried both IJT Base ns and ns=0)"
    )
async def test_calibration_has_certificate_uri(tools_instances, ns_indices):
    ...
    if calibration_node is None:
        pytest.skip("Calibration node not found on first tool")
    cert_node = await find_child_by_browse_name(
        calibration_node, "CertificateUri", ns_ijt
    )
    assert cert_node is not None, (
        "CertificateUri node not found under Calibration"
    )
    value = await cert_node.read_value()
    assert isinstance(value, str), (
        f"CertificateUri value expected str, got {type(value).__name__}: {value!r}"
    )