...
async def test_calibration_node_exists_on_tool(tools_instances, ns_indices):
    ...
    if calibration_node is None:
        pytest.skip(
            "Calibration not found at Maintenance/Calibration — not implemented on this server"
        )
...
async def test_calibration_has_engineering_units(tools_instances, ns_indices):
    ...
    eu_node = await find_child_by_browse_name(
        cal_value_node, "EngineeringUnits", ns_ijt
    )
    if eu_node is None:
