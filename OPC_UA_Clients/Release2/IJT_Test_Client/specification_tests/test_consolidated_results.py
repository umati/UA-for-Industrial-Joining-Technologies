...
def _unwrap_sub_result(item):
    ...
    try:
        ...
        # Guard against malformed items; return unchanged.
        pass  # nosec B110
    return item
...
async def test_intervention_result_content_is_none_or_joining_result(subscription_client, result_trigger, ns_indices):
    ...
    if result_data is None:
        pytest.skip(
            "INTERVENTION_RESULT not generated by current trigger/server — "
            "cannot verify intervention-specific result content"
        )
...
async def test_intervention_result_get_result_by_id_returns_intervention_result(
    ...
    if result_data is None:
        pytest.skip(
            "INTERVENTION_RESULT not generated by current trigger/server — cannot verify GetResultById classification"
        )
