...
@dataclass(frozen=True)
class SecurityMatrixUser:
    username: str
    password: str


@dataclass(frozen=True)
class SecurityMatrixUsers:
    positive: SecurityMatrixUser
    wrong_password: SecurityMatrixUser
    unknown_user: SecurityMatrixUser
...
def write_simulator_user_identity_configuration(
    ...
    *,
    ...
    configured_users[users.positive.username] = {
        ...
        "description": "Security matrix positive user",
    }

    # Simulator default credential for matrix test users. Server-side test fixture
    ...
    configured_users.setdefault(
        users.wrong_password.username,
        {
            ...
            "roles": [],
            "description": "Security matrix wrong-password target",
        },
    )
    ...
    if x509_thumbprint:
        x509_user = configured_users.setdefault(
            x509_user_name,
            {
                ...
                "password": _DEFAULT_SIMULATOR_PASSWORD,
                "roles": [],
                "description": "Security matrix X509 user",
            },
        )
...
def load_security_matrix_users(path: Path) -> SecurityMatrixUsers:
    ...
    def user(section: str) -> SecurityMatrixUser:
        data = sections[section]
        return SecurityMatrixUser(data["username"], data["password"])

    return SecurityMatrixUsers(
