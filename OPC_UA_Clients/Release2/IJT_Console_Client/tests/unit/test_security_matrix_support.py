...
from security_matrix_support import (
...
def test_write_self_signed_certificate_creates_cert_key_and_der(tmp_path: Path) -> None:
    generated = write_self_signed_certificate(
        tmp_path / "certs",
        "user1",
        application_uri="urn:test:security-matrix",
    )
...
def test_load_security_matrix_users_reads_checked_in_yaml_shape(tmp_path: Path) -> None:
...
""".strip(),
    ...
    users = load_security_matrix_users(users_file)
