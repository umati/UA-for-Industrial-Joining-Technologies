...
pytestmark = [pytest.mark.live, pytest.mark.security_matrix]

_CONSOLE_ROOT = Path(__file__).resolve().parent.parent.parent
_REPO_ROOT = _CONSOLE_ROOT.parents[2]
_TMP_ROOT = _CONSOLE_ROOT / "tmp" / "security-matrix"
...
def _client_app_certificate() -> tuple[Path, Path]:
    ...
    if not cert or not key:
        # The security-matrix conftest provisions the client application
        # certificate only when the IJT_SECURITY_MATRIX_CELL env var is set
        # (which is how CI runs each cell). On a plain local run those env
        # vars are absent, so the security-matrix sub-suite is skipped rather
        # than failing loudly.
        pytest.skip(
            "Console security matrix client application certificate not provisioned "
            "(set IJT_SECURITY_MATRIX_CELL to enable this sub-suite)."
        )
...
def _users_file() -> Path:
    ...
    shared = _REPO_ROOT / "OPC_UA_Servers" / "Release2" / "security_matrix.users.yaml"
    if shared.exists():
        return shared
...
async def _assert_certificate_user_token_policy_uses_endpoint_policy(
    ...
    # Force the same skip semantics as the other secure matrix cells on plain
    # local runs where the matrix fixture has not provisioned cert material.
...
async def test_username_happy_path_connects_and_browses(
    ...
    users = load_security_matrix_users(_users_file())
    client = await _connect(
        _security_config(
            security_policy_uri=security_policy_uri,
            security_mode=security_mode,
...
async def test_x509_identity_token_known_thumbprint_user1_opens_session(
    ...
    if not user_cert or not user_key:
        pytest.skip("X509 user-identity cert not provisioned (set IJT_SECURITY_MATRIX_CELL to enable this sub-suite).")
    client = await _connect(
        _security_config(
            security_policy_uri=security_policy_uri,
            security_mode=security_mode,
