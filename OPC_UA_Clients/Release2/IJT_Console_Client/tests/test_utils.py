...
def test_nodeid_matches_web_client_output():
    """Both clients must produce the same string for the same NodeId."""
    import sys
    ...
    web_utils = importlib.import_module("Python.utils")

    node = ua.NodeId(2258, 0)
    assert nodeid_to_str(node) == web_utils.nodeid_to_str(node)

    node2 = ua.NodeId("TighteningSystem", 1, ua.NodeIdType.String)
    assert nodeid_to_str(node2) == web_utils.nodeid_to_str(node2)
