...
def _list_pythons_windows():
    """
    Return versions ['3.16','3.15','3.14',...] enumerated by 'py -0'.
    """
    try:
...
def _find_latest_python_executable():
    """
    Return (cmd_list, "3.X") for the newest Python on this system.
    ...
    if IS_WINDOWS:
        ...
        if not versions:
            log.error(
                ...
                "Please install Python from https://www.python.org/downloads/ or the Microsoft Store,\n"
        ...
        try:
            ...
            log.error(
                "Could not find a usable Python 3 interpreter. Please install Python 3.14+.\n"
...
def _relaunch_under_latest_python():
    """
    If current interpreter is not the newest available, re-exec this script under it.
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# Resolve the concrete interpreter path behind py/python3 (robust venv creation)
# ─────────────────────────────────────────────────────────────────────────────
def _resolve_python_executable(latest_cmd):
    """
    Return absolute path to Python interpreter behind latest_cmd (e.g. 'C:\\...\\python.exe').
    """
    try:
...
# ─────────────────────────────────────────────────────────────────────────────
def _create_venv_with(latest_cmd):
    if VENV_DIR.exists():
        try:
    ...
    try:
        ...
        log.error(
            ...
            "Install Python from python.org (with the 'py' launcher), and verify with:  py -0"
        )
        sys.exit(1)
...
def _install_packages():
    ...
    if req.exists():
        ...
        # Default minimal stack if no requirements.txt
        for pkg in ["pytz", "aiofiles", "orjson"]:
            subprocess.check_call([str(py), "-m", "pip", "install", "--upgrade", pkg])

    # Proactively upgrade crypto stack (asyncua deps often need recent wheels)
    try:
        subprocess.check_call(
    ...
    # Ensure asyncua is at or above the first 3.14‑compatible build; allow pre‑releases
...
def _run_client(url_arg: str, passthrough: list[str] | None):
...
# ─────────────────────────────────────────────────────────────────────────────
def main():
    # Always run under newest Python (Windows: 'py' launcher; POSIX: python3.X probe)
    _relaunch_under_latest_python()

    # Parse args after potential relaunch (so argv is preserved)
    ...
    # Discover newest Python available
    latest_cmd, latest_ver = _find_latest_python_executable()

    # Enforce Python >= 3.14, print loud guidance if not satisfied
    try:
    ...
    # Create venv with the resolved interpreter and install deps
    _create_venv_with(latest_cmd)
    _install_packages()

    # Decide URL and run client
