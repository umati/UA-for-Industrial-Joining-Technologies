...
async def log_event_details(
    ...
    ijt_log.info(f"{'Client Time':<{label_width}} : {formatted_client_time}")
    ijt_log.info(f"{'Server Time':<{label_width}} : {formatted_server_time}")
    ijt_log.info(f"{'StartTime of Tightening':<{label_width}} : {formatted_start_time}")
    ijt_log.info(f"{'EndTime of Tightening':<{label_width}} : {formatted_end_time}")
    ijt_log.info(f"{'Event Generated Time':<{label_width}} : {formatted_event_time}")
    if latency_ms is not None:
        ijt_log.info(
            f"{'Turnaround Time (EndTime → Client)':<{label_width}} : {abs(latency_ms):.3f} ms"
        )
    else:
        ijt_log.info(
            f"{'Turnaround Time (EndTime → Client)':<{label_width}} : Unavailable"
        )
    ijt_log.info("-" * 80)
    return event_id
